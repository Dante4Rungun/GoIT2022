...
const saveData = (0, _lodashThrottleDefault.default)(()=>{
    localStorage.setItem("feedback-form-state", JSON.stringify(messageData));
}, 500);
loadLustFormData();
form.addEventListener("input", (event)=>{
    event.preventDefault();
    if (event.target.getAttribute("name") === "email") messageData.email = event.target.value;
    if (event.target.getAttribute("name") === "message") messageData.message = event.target.value;
    saveData();
});
...
},{}]},["bMXDs","eH52W"], "eH52W", "parcelRequired7c6")
